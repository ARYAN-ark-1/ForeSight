...
const CryptoDetails = () => {
  ...
  //   navigate("/result",{ state: { tickerSymbol: data.symbol } });
  // };
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch('http://localhost:5000/analyse', {
        ...
        body: JSON.stringify({
          tickerSymbol: 'AAPL', // or dynamically get the ticker symbol from input
      ...
      if (result.success) {
        ...
        window.open(url, "_blank"); // Open the Flask page in a new tab// Redirect to the /index page to display the plot
      } else {
  ...
  //     });
  //     window.open(response.request.responseURL, "_blank"); // Open the rendered HTML page in a new tab
  //   } catch (error) {
                  ...
                  <>
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: "#1DA1F2", // Example color, adjust as needed
                        color: "#ffffff",
                      }}
                    >
                      Predict
